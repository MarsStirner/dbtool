...
def upgrade(conn):
    ...
    sql = u'''CREATE DEFINER=%s PROCEDURE `form007front`(IN end_date VARCHAR(128), IN org_str VARCHAR(128) , IN profiles VARCHAR(128))
              BEGIN
SET @tpl = "select
	rbhbp.name,
	total.count_koikas,
...
SET @sqlPrepare = REPLACE(REPLACE(REPLACE(@tpl, "::@end_date", end_date),"::@org_str",org_str),"::@profiles",profiles);
