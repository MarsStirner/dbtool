...
__doc__ = '''\
Конвертирование значений t из строкового представления в тип Double.
Для каждого типа свойства действия с именем t создается свойство типа действия t
Для каждого значения t из ActionProperty_String
...
sqlSelectStringTemperatureValues = '''\
...
def upgrade(conn):
    ...
    # Выбираем строковые значения для свойства t
    res = query(conn, sqlSelectStringTemperatureValues)
    for row in res:
        actionId = row[0]
