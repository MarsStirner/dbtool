...
def upgrade(conn):
    ...
    sqlsWithCheck.append((u"""select id from ActionPropertyType
            ...
            values((select id from ActionType at where at.name=\"Назначения\" AND deleted=0 LIMIT 1),
    ...
    sqlsWithCheck.append((u"""select id from ActionPropertyType
            ...
            set deleted = 1
            where actionType_id IN (select id from ActionType at where at.name=\"Назначения\")
            and name = \"Доза\" and
            typeName = \"String\" """
        ))
    sqlsWithCheck.append((u"""select id from ActionPropertyType
